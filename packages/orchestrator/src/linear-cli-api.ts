...
export function createLinearCliApi(cliPath?: string): LinearApi {
  ...
  return {
    ...
    async listChildIssues(parentId: string): Promise<RawIssue[]> {
      ...
      // linear-cli issue list needs the internal UUID for parent-id filtering
      // but returns the readable ID (ENG-XXX) in the id field.
      // We need the internal UUID — check if parentId is already a UUID or a readable ID
